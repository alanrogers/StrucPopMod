...
d = 11
N = 600 # deme size
M = 6 # rate of migration per pair of lineages per 2N generations
...
for i in range(d):
    for j in range(d):
        if i == j:
            continue
        dem.set_migration_rate(i, j, m/(d-1))
...
while True:
    ...
    if t > max_t:
        break
    i, j = sample(range(d), 2)
    dem.add_mass_migration(t, source=i, dest=j, proportion=1)
    n_extinctions += 1
